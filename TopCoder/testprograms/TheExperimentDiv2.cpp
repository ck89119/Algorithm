...
public:
	...
	vector <int> determineHumidity(vector <int> intensity, int L, vector <int> leftEnd) {
	    ...
	    int i,j;
		for (i=0;i<m;i++){
            sponge[i].leftend=leftEnd[i];
            sponge[i].no=i;
		}
		sort(sponge,sponge+m,cmp);
		for (i=0;i<m;i++)
            for (j=0;j<L;j++)
                cov[sponge[i].leftend+j]=sponge[i].no;
        vector<int> res(m);
        for (i=0;i<m;i++) res[i]=0;dump(m)
        for (i=0;i<m;i++){
            int j=sponge[i].leftend;
            int k=0;
                    //dump("#######")
            while (k<L){
                res[j]+=intensity[j];
