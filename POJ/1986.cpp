...
int get_length(int u, int len) {
  vis[u] = 1; length[u] = len;
  for (int i = 0; i < edges[u].size(); ++i) {
    int v = edges[u][i].first;
    int w = edges[u][i].second;
    if (!vis[v]) get_length(v, len + w);
  }
  return 0;
}
...
int main() {
  ...
  for (int i = 0; i < ans.size(); ++i) {
    ...
    if (w == -1) {
      for (int j = 0; j < querys[v].size(); ++j)
        if (querys[v][j].first == u) {
          w = querys[v][j].second;
          break;
        }
    }
    printf("%d\n", length[u] + length[v] - 2*length[w]);
  }
  return 0;
}
