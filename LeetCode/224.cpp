...
class Solution {
 public:
  ...
  int calculate(string s) {
      ...
      if (is_digit(s[cur])) {
        ...
        if (oper == '+' || oper == '-' || oper == '(') {
          ...
          while (!op.empty() && op.top() != '(') {
            tmp_nums.push(nums.top()); nums.pop();
            tmp_op.push(op.top()); op.pop();
          }
          op.pop();
          tmp_nums.push(nums.top()); nums.pop();
          while (!tmp_op.empty()) {
            int n1 = tmp_nums.top(); tmp_nums.pop();
            int n2 = tmp_nums.top(); tmp_nums.pop();
            char oper = tmp_op.top(); tmp_op.pop();
            tmp_nums.push(oper == '+' ? n1 + n2 : n1 - n2);
          }
          nums.push(tmp_nums.top()); tmp_nums.pop();
        }
        ++cur;
      }
    }
    ...
    if (tmp_nums.empty()) return 0;
    else return tmp_nums.top();
  }
};
